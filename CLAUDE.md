...
- **database/migrations/** — SQL escrito à mão `NNN_nome.sql`, aplicado por `backend/scripts/apply-sql.mjs` (usa `backend/.env` → `DATABASE_URL`). **Toda migration é um arquivo `.sql`; ANTES de criar, verifique o ÚLTIMO número existente em `database/migrations/` e crie o PRÓXIMO na sequência** (`NNN` em ordem, sem pular nem repetir). Idempotente sempre (`create ... if not exists`, guardas); tabela só-nuvem leva `-- @cloud-only` no início da linha. Aplicação: **usuário aplica na NUVEM; eu testo no local.**
...
- **⚠️ A `main` LOCAL de `C:\Regen` fica DEFASADA — `origin/main` é a fonte da verdade.** Os merges acontecem no GitHub (PR/`gh pr merge`) → só a `origin/main` avança; a `main` local **não** é fast-forwardada sozinha e o **WIP não-commitado bloqueia `git pull`**. Já ficou **158 commits atrás** (16/08→29/08) — foi o que fez a migration 218 "sumir" e o `delivery.service` aparecer com −268 linhas. **Regra:** (1) sempre `git fetch` e referenciar **`origin/main`** para qualquer verdade — numeração de migration, "isso existe?", build, corte de release — **nunca** os arquivos do working tree local; (2) **buildar/cortar release SÓ de worktree off `origin/main`** (`git worktree add`); (3) ao mesclar, **trazer o arquivo pro `C:\Regen`** (`git show origin/main:path > path`) ou fast-forward da main local com a árvore limpa (`git merge --ff-only origin/main`); (4) manter WIP longo em **branch**, não solto na `main`.
...
- **Modelo de distribuição — dois planos de acesso (usuário × distribuição Regem).** O projeto é um produto DISTRIBUÍDO/revendável. Toda lógica nasce separando o que é do **USUÁRIO** (loja/cliente) do que é da **DISTRIBUIÇÃO** (Regem). O usuário acessa só os PRÓPRIOS dados de negócio (sua loja, colaboradores, vendas, cardápio, clientes…). Informação **lógica de código, segredos, credenciais/config de integração, telemetria, dados de outras lojas, painel/console de distribuição** é a cargo da **distribuição Regem** e **NUNCA** exposta nem editável pelo usuário (nem na API, nem no front). Na dúvida sobre a quem um dado pertence, ele é da distribuição.
- **Integração = o usuário informa, a Regem conclui.** Em qualquer integração (iFood, 99Food, Keeta, WhatsApp, gateways de pagamento…), o usuário **NÃO cria app/conta de dev nem manuseia segredo**. Ele só informa os dados estritamente necessários **quando NÓS solicitarmos**; a criação do app, a configuração, a guarda de segredos e a publicação são concluídas por nós no lado da distribuição. O fluxo do usuário pede o mínimo — o resto é nosso, nos bastidores.
- **Escala/concorrência é premissa, não otimização depois.** Toda lógica nasce assumindo MUITOS usuários e lojas usando o sistema **ao mesmo tempo**: multi-tenant sempre; nada de estado global compartilhado entre requisições (sem singleton mutável por request); operações **idempotentes** e transacionais (locks/`LWW` corretos onde há concorrência real); operação em massa = **1 request + query set-based**, nunca loop de N; nenhum caminho que serialize ou trave os demais usuários.
...
**Pendente** (tem mockup, falta implementar): grade **semanal** de escala, **linha do tempo operacional**, **PVPS/lotes/validade** + **recebimento de nota**, **Pessoas & Ponto** (gerencial), **Fornecedores**, **Log de Auditoria**, e as frentes **Vendas & Comandas + Financeiro**. Apps satélites **KDS** e **Ponto** (independentes, WebSocket). *(Entregues: Mural & Clima `/mural` mig.026; Bot de Suporte `/bot` mig.027; Wizard por ramo rico `/wizard` sem mig.; Fichas aninhadas mig.028; upload de mídia real já plugado em todas as telas de foto.)*

## Releases (edge `.exe`/`.zip` e APKs) — `RELEASES.md` na raiz

- **`RELEASES.md` (raiz) é a fonte da verdade** de tudo que é empacotado/distribuído: instalador (`.exe`) e atualização (`.zip`) do edge, e APKs. **Consultar antes de gerar** qualquer artefato; **atualizar ao mesclar** cada PR com mudança distribuível.
- **Não empacotar a cada alteração.** Em fase de criação/testes/correção, registrar a pendência em `RELEASES.md` e **acumular**. Só cortar release (gerar `.zip`/`.exe`/`.apk`) quando acumular vários PRs ou o usuário pedir.
- Ao cortar: **`.zip`** sempre (mudança de backend/edge); **`.exe` novo só se tocou `sync` ou `edge`** (scripts `backend/edge/*`, `sync-config`/`sync-daemon`, deps embutidas, `.iss`) ou para manter instalação nova na versão corrente; **`.apk`** só após testes. O **`.exe` sai sempre versionado com a última versão** (`regem-edge.iss` `AppVer`).
- Publicação do edge: `edge/publicar.ps1` → subir `.zip` no Supabase Storage (bucket `edge-updates`, nome exato `regem-edge-X.Y.Z.zip`) → publicar no **console de distribuição** (grava `edge_release`, vence as envs do EasyPanel).
...
5. **Ao mesclar um PR com mudança distribuível, registrar a pendência em `RELEASES.md`** (sem empacotar). Empacotar só ao cortar release.
