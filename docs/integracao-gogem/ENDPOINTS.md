...
- **Chamada:** `POST {GOGEM_CLOUD_URL}/sync/regem/pedido-cancelado` (padrão `https://api.gogem.com.br/api/v1`), `X-Sync-Token` = **o token da integração do GoGeM** — o do equipamento que o **próprio GoGeM marcou** (mig 290, ERR-108): toda chamada do GoGeM à nuvem vem com `X-Integrador: gogem` junto do token, e o `SyncTokenGuard` marca aquele `servidor_local` (`equipamento.integrador = 'gogem'`; grava só quando muda ou de hora em hora; servidor de loja que já sincronizou não vira credencial). Escolha: o marcado e ativo da loja do pedido, depois o da rede, depois qualquer um da empresa; no empate, o visto por último. **Sem marcado, não se chuta** um `servidor_local` qualquer (no piloto, 5 de 6 davam 401): o aviso espera ("o GoGeM ainda não se identificou para esta empresa") e a marca solta a fila na hora. O "Publicar no GoGeM" usa o mesmo token.
- **Servidor da loja → nuvem → GoGeM:** **só a nuvem fala com o GoGeM** — o token da integração não mora na loja (credencial de integração é da distribuição). O servidor da loja grava o aviso na fila dele e o repassa para `POST {CLOUD_API}/gogem/avisos/pedido-cancelado` (rota só da nuvem, `X-Sync-Token` = `SYNC_TOKEN` da loja; corpo `{ avisoId, chave, corpo, referenciaTipo, referenciaId }`). A nuvem grava com o **mesmo id** (repetir o repasse devolve o mesmo aviso; a mesma venda não avisa duas vezes), tenta o GoGeM na hora (5 s) e responde `{ aceito: true, avisoId, estorno: { situacao, mensagem } }`. Para a loja, aceito = entregue; a nuvem segue com o aviso até o GoGeM confirmar. Sem internet, ele espera na fila da loja e sai com recuo.
- **Corpo:** `{ "idempotencyKey": "<chave do totem>", "regemComandaId": "<comanda — só se houver>", "motivo": "<texto>" }` (limites do DTO do GoGeM: 120 / 120 / 500, cortados ANTES de enviar sem partir caractere; o `motivo` vai cru, sem o "autorizado por…" do hub; nenhum campo além desses três — o GoGeM recusa com 400). No cupom, `idempotencyKey = comanda.idempotency_key`; no hub, `= pedido_externo.external_id` — a mesma chave, então a mesma venda cancelada pelos dois caminhos gera **um** aviso.
...
- **Leitura da resposta:** `200` + `estorno.feito=true` → entregue ("estorno solicitado ao Mercado Pago"); `feito=false` com meio `dinheiro`/`desconhecido` → entregue ("devolva no balcão"); `feito=false` com meio **eletrônico** → o Mercado Pago falhou naquela hora: **reenviar** (o GoGeM tenta o estorno de novo no aviso repetido); `401/403` → aguarda a integração (alerta uma vez, nova tentativa a cada 6 h — ou na hora, quando o GoGeM se identificar de novo); `408/429/5xx`/sem resposta → passageiro: reenviar com recuo, respeitando o `Retry-After` quando vier (vale o maior), sem alerta; outros `4xx` → recusado com alerta (estorno manual); 7 dias sem confirmação → alerta de estorno manual.
- **Limite do GoGeM (120 requisições/min por IP):** na nuvem os avisos de todas as lojas saem do mesmo IP. O job manda no máximo 20 por minuto, e um `429` no meio do lote PARA o lote: os avisos que não saíram voltam à fila sem contar tentativa e esperam o `Retry-After` (mínimo 1 min).
