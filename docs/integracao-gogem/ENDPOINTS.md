...
- **Leitura da resposta:** `200` + `estorno.feito=true` → entregue ("estorno solicitado ao Mercado Pago"); `feito=false` com meio `dinheiro`/`desconhecido` → entregue ("devolva no balcão"); `feito=false` com meio **eletrônico** → o Mercado Pago falhou naquela hora: **reenviar** (o GoGeM tenta o estorno de novo no aviso repetido); `401/403` → aguarda a integração (alerta uma vez, nova tentativa a cada 6 h); `408/429/5xx`/sem resposta → passageiro: reenviar com recuo, respeitando o `Retry-After` quando vier (vale o maior), sem alerta; outros `4xx` → recusado com alerta (estorno manual); 7 dias sem confirmação → alerta de estorno manual.
