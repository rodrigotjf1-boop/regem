...
2. ✅ **FEITA (18/07/2026) — Painel Frota + Telemetria**: `GET /distribuicao/frota` (empresas + último `edge_heartbeat`: versão/online/clientes/erro + licença + nº erros abertos; todos os perfis) e `GET /distribuicao/telemetria` + `POST .../:id/resolver` (Diretoria+Técnico; Financeiro não vê). Home com abas Frota/Telemetria/Usuários. Smoke-test OK. *(PII na telemetria: redação fica p/ hardening.)*
3. **Licenças/Leases + Financeiro** — reaproveitar o modelo de lease/licença existente.
4. ✅ **FEITA (19/07/2026) — Ações remotas + fecho**: **Publicar release** pelo console (mig 124 `edge_release`; o `update-check` lê o último release do DB, fallback env → dispensa editar env no EasyPanel; aba Atualizações). **Rollback remoto** (`edge_comando` mig 124; console enfileira, o `sync-daemon` busca `GET /edge/comandos` e executa via `schtasks RegemEdgeRollback`, confirma em `/ack`; botão na Frota). Aba **Auditoria**. Lente **Financeiro** (assinatura na Licenças). **PII redigida** na telemetria (email/CPF/CNPJ/telefone). ⚠️ lado-edge do rollback precisa validar em edge real. Falta (hardening/opcional): MFA + IP allowlist; deletar tenants de teste (hoje ocultos).
