...
- [x] Casca Electron (`desktop/`) endurecida (`contextIsolation`/`sandbox`/`nodeIntegration:false`/`webSecurity`; DevTools off em prod; kiosk; menu removido; nav externa e nova janela bloqueadas). **Modo único local-first**: fala com o Servidor (LAN/localhost) e cai pra nuvem com *status dot* (local/nuvem/offline) voltando ao local sozinho. Config por `regem-desktop.json`/envs.
- [x] **Assinatura Authenticode — rota CA interna (custo zero).** `edge/gerar-ca-assinatura.mjs` gera a CA de code-signing da distribuição: `code-signing-ca.pem` (público, versionado, vai no pacote) + `code-signing.pfx` (segredo, gitignored). O instalador (`Confia-AssinaturaCA`) confia a CA em `LocalMachine\Root`+`TrustedPublisher` nos dois modos → app/instalador assinado não cai em "editor desconhecido" na frota. **CA gerada (CN "DMS Regem", 01/08/2026).** Assina-se o `.exe`/app com o `.pfx` (`CSC_LINK`/`CSC_KEY_PASSWORD` no electron-builder, ou `signtool` no Inno Setup). Selo "publisher verificado" p/ download público na internet = Certum OV (rota futura, ver `desktop/README.md`).

> **ROADMAP CONCLUÍDO** — Fases 0–6 + pendências (fingerprint forte, restore assistido, reconciliação de caixa) + assinatura por CA interna. O sistema local-first está pronto para produção; resta operacional: publicar o pacote de update e compilar/assinar o instalador `.exe`.
