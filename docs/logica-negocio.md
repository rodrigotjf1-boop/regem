...
| 2026-07-03 | **Passo 3** (branch `feature/jobs-estoque`, sem migration): ROP real no `/estoque/inteligencia` (ES = CMD × `dias_seguranca` + `qtdSugerida`); view **`/estoque/validades`** (FEFO por vencimento, status crítico/atenção/vencido); jobs **06:00 (ROP)** e **06:10 (FEFO)** emitem alerta em tempo real ao KDS/gestor conectado. Alertas **efêmeros** (persistência = migration futura); FEFO usa `lote.quantidade` (saídas ainda não decrementam lotes). | §8 passo 3 |
| 2026-07-03 | **Passo 4** (branch `feature/cmv-real`, sem migration): `POST /estoque/snapshot` + cron mensal (dia 1, 02:00) grava `estoque_snapshot`; `GET /estoque/cmv` calcula **CMV real** (EI+Compras−EF via snapshots; EF cai p/ valorização atual se sem snapshot) × **CMV teórico consumido** (saídas venda/produção) → **desvio**; card no `/estoque`. **Furo (desvio − desperdício) fica pendente:** desperdício não liga a item/custo. | §8 passo 4 — o KPI de ouro |
| 2026-07-03 | **Passo 6** (branch `feature/jornada`, sem migration — usa `feriado` da 024): `/ponto/espelho` agora traz **prévia gerencial** de jornada (§3): **extra** (50% dia útil, 100% domingo/feriado), **adicional noturno** (faixa 22h–5h, hora local BR UTC−3) e **DSR estimado sobre extras**; funções puras `minutosNaFaixaNoturna`/`fatorHoraExtra` (testes §7 caso 7); bloco + colunas no espelho PDF com disclaimer "confira com a contabilidade". **Passo 5 (fluxo de caixa) já existia** (Fase H2, `/financeiro`). | §8 passo 6 |
| 2026-07-03 | **Furo de CMV** (branch `feature/furo`, **migration 025**: `desperdicio` + `item_id`/`custo_unitario`): desperdício **com item** agora **baixa o estoque** (movimento saída `motivo='desperdicio'` ao custo médio, em transação) e é valorizado — corrige o desperdício nunca ter movido estoque. `GET /estoque/cmv` + `desperdicioValor` e **`furo = desvio − desperdício`** (§1.3); função pura `furoCmv` + testes (33 no total). Front: seletor de item opcional em `/operacao` + valor da perda; card CMV mostra Desperdício e Furo. Desperdício em texto livre segue só log (compatível). | Fecha o ciclo desvio→desperdício→furo |
| _adicionar aqui_ | | |
