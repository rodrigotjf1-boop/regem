...
// ausente). Antes o /frota mostrava só 🟢/⚪ pelo heartbeat: Pg/Impressão caídos = verde.
const SERVS_EDGE: [string, string][] = [['pg', 'PG'], ['api', 'API'], ['sync', 'Sync'], ['impressao', 'Impr'], ['web', 'Web']];
function saudeBadges(f: any) {
  const s = f?.saude?.servicos;
  if (!s) return <span className="text-xs text-muted-foreground">—</span>;
  return (
    <div className="flex flex-wrap gap-1">
      {SERVS_EDGE.map(([k, label]) => {
        const ok = s[k] === 'Running';
        return (
          <span key={k} title={`${label}: ${s[k] ?? '?'}`}
            className={`rounded px-1.5 py-0.5 font-mono text-[10px] ${ok ? 'bg-ok/15 text-ok' : 'bg-destructive/15 text-destructive'}`}>
            {label}
          </span>
        );
      })}
